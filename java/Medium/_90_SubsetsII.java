...
public class _90_SubsetsII {
    public List<List<Integer>> subsetsWithDup(int[] nums) {
        ...
        for(int cnt = 0; cnt<=nums.length; cnt++){
            comb(nums,0,cnt,new ArrayList<>(), res);
        }
        
        return res;
    }  

    public void comb(int[] nums, int start, int cnt,List<Integer> prt, List<List<Integer>> res){
        if(cnt == 0){
            res.add(new ArrayList<>(prt));
            return;
        }
        
        for(int i = start; i<nums.length-(cnt-1); i++){
            if(i>start && nums[i] == nums[i-1]) continue;
            prt.add(nums[i]);
            comb(nums,i+1,cnt-1,prt,res);
